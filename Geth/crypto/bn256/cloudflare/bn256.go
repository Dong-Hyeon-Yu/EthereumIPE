...
func Fp12mul(a, b []byte) (*GT, error) {
	ans := new(gfP12)
	...
	_, err := tempA.Unmarshal(a)
	if err != nil {
		return nil, err
	}
	_, err = tempB.Unmarshal(b)
	if err != nil {
		return nil, err
	}

	ans.Mul(tempA.p, tempB.p)
...
// Pairing calculates the Optimal Ate pairing for a set of points.
func Pairing(a []*G1, b []*G2) []byte {
	acc := new(gfP12)
	acc.SetOne()
	...
	var final = finalExponentiation(acc)
	var result = &GT{final}
