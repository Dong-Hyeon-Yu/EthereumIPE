...
func TestFp12mul(t *testing.T) {
	...
	const fp12Size = 32 * 12

	var (
		zero = make([]byte, fp12Size) // all zero
		one  = make([]byte, fp12Size)
	)
	one[fp12Size-1] = 1

	// 0x0 = 0
	if res, _ := Fp12mul(zero, zero); !res.p.IsZero() {
		t.Fail()
	}
	// 1x0 = 0
	if res, _ := Fp12mul(one, zero); !res.p.IsZero() {
		t.Fail()
	}
	// 0x1 = 0
	if res, _ := Fp12mul(zero, one); !res.p.IsZero() {
		t.Fail()
	}
	// 1x1 = 1
	if res, _ := Fp12mul(one, one); !res.p.IsOne() {
		t.Fail()
	}
}

func TestFp12mulPairing(t *testing.T) {

	// case1: Pair(g1, g2) * Pair(g1, g2) == Pair(2*g1, g2)
	g1 := new(G1).ScalarBaseMult(big.NewInt(1))
	g2 := new(G2).ScalarBaseMult(big.NewInt(1))

	res, _ := Fp12mul(Pair(g1, g2).Marshal(), Pair(g1, g2).Marshal())
	expected := Pair(new(G1).ScalarMult(g1, big.NewInt(2)), g2)

	if !bytes.Equal(res.Marshal(), expected.Marshal()) {
		t.Fail()
	}

	// case2: Pair(2*g1, g2) * Pair(3*g1, g2) == Pair(5*g1, g2)
	res, _ = Fp12mul(
		Pair(new(G1).ScalarMult(g1, big.NewInt(2)), g2).Marshal(),
		Pair(new(G1).ScalarMult(g1, big.NewInt(3)), g2).Marshal())
	expected = Pair(new(G1).ScalarMult(g1, big.NewInt(5)), g2)
